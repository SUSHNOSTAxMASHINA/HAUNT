...
import { compactPanel as asciiCompactPanel, renderASCIIField, } from "../src/hauntASCII.ts";
import { renderBrailleSubliminal, renderSpectre, } from "../src/spectre.ts";
...
const POSSESS_STATES = ["waiting", "writing", "thinking"];
const DITHER = [".", ":", ";", "+", "*", "#", "@", "#"];
...
function inkColor(index, brightness) {
...
function ghostPixel(px, py, width, height, frame, activity = "waiting") {
    ...
    const flow = Math.sin(ny * 8 + phase * 1.6) * 0.055 * profile.turbulence +
    ...
    const memory = Math.exp(-((memoryY - Math.sin(memoryX * 8 + memoryPhase) * 0.16) ** 2) / 0.05) *
    ...
    const halo = Math.exp(-((x + 0.03) ** 2 / 0.58 + (y + 0.02) ** 2 / 1.2)) * 0.34;
    const ribbon = Math.exp(-((y - Math.sin(x * 7 + phase) * 0.16) ** 2) / 0.025) *
        (0.3 + profile.beam * 0.18);
    const lowerCoil = Math.exp(-((y - 0.48 - Math.sin(x * 9 - phase * 1.4) * 0.1) ** 2) / 0.05) *
        Math.max(0, 1 - Math.abs(x) * 1.4) *
        (0.72 + profile.turbulence * 0.28);
    const voidBand = Math.exp(-((y + 0.11 - Math.sin(x * 5 + phase) * 0.08) ** 2) / 0.018) *
        0.42;
    const pressure = Math.max(0, Math.sin(phase * 0.31 - 2.4)) ** 4;
    const breathRing = Math.exp(-((Math.sqrt(x * x + y * y) - (0.72 + pressure * 0.18)) ** 2 / 0.045)) *
        pressure *
        0.07;
    ...
    const gaze = Math.sin(phase * 0.19 + 1.3) * 0.018 + Math.sin(phase * 0.047) * 0.012;
    const eyeLeft = Math.exp(-(((x + 0.27 - gaze) / 0.16) ** 2 + ((y + 0.16) / 0.07) ** 2));
    const eyeRight = Math.exp(-(((x - 0.14 - gaze) / 0.16) ** 2 + ((y + 0.16) / 0.07) ** 2));
    const blink = Math.max(0, Math.sin(phase * 0.29 + 0.8)) ** 18;
    const eyeOpenness = 1 - blink * 0.92;
    const eyeVoid = (Math.exp(-(((x + 0.27 - gaze) / 0.08) ** 2 + ((y + 0.16) / 0.035) ** 2)) +
        Math.exp(-(((x - 0.14 - gaze) / 0.08) ** 2 + ((y + 0.16) / 0.035) ** 2))) *
        eyeOpenness;
    const eyeTrace = (eyeLeft + eyeRight) * eyeOpenness;
    const mouth = Math.exp(-((y - 0.34 - Math.sin(x * 8 + phase) * 0.025) ** 2 / 0.022)) *
        Math.exp(-((x + 0.06) ** 2) / 0.24);
    const figureTurn = Math.sin(phase * 0.11 - 0.6) * 0.045;
    const figureX = x - 0.68 - figureTurn * (y + 0.1);
    const figureHead = Math.exp(-((figureX / 0.2) ** 2 + ((y + 0.34) / 0.28) ** 2));
    const figureShoulders = Math.exp(-((y - 0.02 - Math.sin(figureX * 5 + phase) * 0.04) ** 2) / 0.06) *
        Math.exp(-(figureX ** 2) / 0.38);
    const peripheralFigure = faceReveal * (figureHead * 0.06 + figureShoulders * 0.05) * 0.7;
    let density = Math.max(0, core * 0.82 +
        halo +
        ribbon +
        lowerCoil +
        breathRing +
    ...
    const scan = 0.68 + 0.32 * Math.sin(((py + frame * 0.7 * profile.speed) * Math.PI) / 3);
    const interference = Math.sin(x * 42 + y * 15 + phase * 2.3) * 0.12 * profile.turbulence;
    ...
    const edgeLoss = Math.max(0, Math.abs(x) - 0.58) * (0.08 + profile.turbulence * 0.05);
    const packetLoss = noise(px * 0.7, py * 0.9, Math.floor(frame / 3) + 31) < edgeLoss;
    if (packetLoss)
        density *= 0.3;
    const tear = Math.sin(y * 30 + phase * 2.1) > 0.76 &&
    ...
    const band = Math.floor((x + 1 + Math.sin(phase * 0.4) * 0.16) * 2.8 +
        Math.sin(y * 4 + phase) * 1.4 +
        profile.inkShift);
    return {
        density: Math.max(0, Math.min(1, density)),
        color: inkColor(band + (tear ? 2 : 0), Math.min(1.35, leftLight + density * 0.5)),
    };
}
function ditherPixel(pixel, x, y, frame) {
    const threshold = 0.24 +
        BAYER[((y + Math.floor(frame / 3)) % 4) * 4 + ((x + Math.floor(frame / 5)) % 4)] /
            34;
...
function cellColor(samples, mask, foreground) {
    let red = 0;
    let green = 0;
    let blue = 0;
    let count = 0;
...
function terminalCell(cellX, cellY, width, frame, previousFrame, activity = "waiting") {
    const samples = [];
    let desiredMask = 0;
    for (let y = 0; y < CELL_HEIGHT; y++) {
        for (let x = 0; x < CELL_WIDTH; x++) {
            ...
            const pixel = ghostPixel(pixelX, pixelY, width * CELL_WIDTH, MACHINE_HEIGHT * CELL_HEIGHT, frame, activity);
            samples.push(pixel);
            desiredMask =
                ((desiredMask << 1) |
    ...
    if (desiredMask === 0) {
        const old = ghostPixel(cellX * CELL_WIDTH + 2, cellY * CELL_HEIGHT + 4, width * CELL_WIDTH, MACHINE_HEIGHT * CELL_HEIGHT, previousFrame, activity);
    ...
    return {
        ...
        background: pattern.mask === 0xffffffff
            ? undefined
            : (cellColor(samples, pattern.mask, false) ?? VOID),
    };
}
function terminalCellRow(rowIndex, width, frame, previousFrame, activity = "waiting") {
    ...
    for (let cellX = 0; cellX < imageWidth; cellX++) {
        const cell = terminalCell(cellX, rowIndex, imageWidth, frame, safePreviousFrame, activity);
        result +=
...
function machineFieldRows(width, frame, activity, count, whisper, interaction = "none", interactionStrength = 1, visualizer = "braille", paletteFromState = activity, paletteBlend = 1, animationFromState = activity, animationBlend = 1, paletteToState = activity, animationToState = activity, rows = MACHINE_FIELD_ROWS) {
    const imageWidth = visualizer === "braille"
        ? Math.max(1, width)
        : Math.min(MACHINE_WIDTH, Math.max(1, width));
    const left = Math.floor((width - imageWidth) / 2);
    const right = Math.max(0, width - left - imageWidth);
    const lines = visualizer === "ascii"
        ? renderASCIIField(imageWidth, Math.max(1, rows), Math.floor(frame))
        : renderBrailleSubliminal(imageWidth, Math.max(1, rows), frame, activity, count, interaction, interactionStrength, whisper, "large", paletteFromState, paletteBlend, animationFromState, animationBlend, paletteToState, animationToState);
    return lines.map((line) => `${" ".repeat(Math.max(0, left))}${line}${" ".repeat(right)}`);
}
function machineFrame(frame) {
    const cx = 34;
    const cy = 10;
    return Array.from({ length: MACHINE_HEIGHT }, (_, y) => Array.from({ length: MACHINE_WIDTH }, (_, x) => {
        ...
        if (codeSignal && skull > 0.72)
            return "01{}[]<>/\\|=+*#@$%&"[Math.floor(noise(x + 19, y + 7, frame + 6) * 17)];
        const index = Math.max(0, Math.min(GLYPHS.length - 1, Math.round(Math.max(0, Math.min(1, density)) * (GLYPHS.length - 1))));
        return GLYPHS[index];
    }).join(""));
}
const TRACE = [
...
export function compactPanel(width, frame, state, count, interaction = "none", interactionStrength = 1, whisper = false, paletteFromState = state, paletteBlend = 1, animationFromState = state, animationBlend = 1, paletteToState = state, animationToState = state, textFrame = frame) {
    return renderSpectre(width, 5, frame, state, count, interaction, interactionStrength, whisper, paletteFromState, paletteBlend, animationFromState, animationBlend, paletteToState, animationToState, textFrame);
}
...
function activityFor(state, thinking, toolActive, preferred) {
    if (preferred)
        return preferred;
...
function activityThought(activity, frame, count, possessView = false) {
...
function glitchTitle(_frame) {
    return TITLE;
}
function glitchText(text, frame) {
    const marks = "░▒▓█·";
    const tick = Math.floor(frame);
    return Array.from(text)
        .map((character, index) => character !== " " && (index * 7 + tick) % 13 < 2
...
export function panel(width, frame, previousFrame = frame - 1, attentionActive = false, attentionCount = 0, thinking = false, echo = "", transition = 0, toolActive = false, toolCount = 0, errorActive = false, idleActive = false, idleCount = 0, memoryCount = 0, sessionActive = false, sessionCount = 0, interaction = "none", interactionStrength = 1, knockSeed = 0, whisper = false, visualizer = "braille", paletteFromState, paletteBlend = 1, preferredActivity, animationFromState, animationBlend = 1, paletteToState, animationToState, fieldRows = MACHINE_FIELD_ROWS, possessView = false) {
    const w = Math.max(1, Math.floor(width || 1));
    const textFrame = Math.floor(frame);
    const mood = ["listening", "remembering", "watching", "almost awake"][Math.floor(textFrame / 2) % 4];
    const activityState = activityFor({ attentionActive, errorActive, sessionActive, idleActive }, thinking, toolActive, preferredActivity);
    const thought = activityThought(activityState, textFrame, attentionCount + toolCount + sessionCount + idleCount, possessView) +
        (memoryCount > 0 && textFrame % 11 < 3
            ? ` / ${MEMORY_MARKS[(memoryCount + textFrame) % MEMORY_MARKS.length]}`
            : "");
    const smallSubliminal = "";
    const largeWhisper = activityThought(activityState, textFrame + 1, attentionCount + toolCount, possessView);
    ...
    if (w < 42) {
        return [
            centered(TITLE, w),
            row(`${sessionActive ? "↺" : idleActive ? "?" : errorActive ? "※" : toolActive ? "◇" : stateGlyph(attentionActive, thinking, transition)} ${mood} / ${thinking ? "thinking" : breathLabel(textFrame)}`, w),
            row(smallSubliminal, w),
            row(`${sessionActive ? sessionWeatherLabel(textFrame, sessionCount) : errorActive ? errorWeatherLabel(textFrame) : toolActive ? sensation : echo}`, w),
        ];
    }
    ...
    const dither = Array.from({ length: Math.max(0, w - 12) }, (_, i) => DITHER[(i + textFrame * (activityState === "error" ? 3 : 1)) % DITHER.length]).join("");
    ...
    const breath = activityState === "thinking"
        ? "compressing"
    ...
    const idleText = idleActive
        ...
        : "";
    const memoryMark = MEMORY_MARKS[(memoryCount + textFrame) % MEMORY_MARKS.length];
    const sessionWeather = SESSION_WEATHER[(textFrame + sessionCount) % SESSION_WEATHER.length];
    const now = new Date();
    ...
    const field = machineFieldRows(w, frame, activityState, attentionCount +
        toolCount +
        sessionCount +
        (interaction === "knock" ? knockSeed : 0), whisper, interaction, interactionStrength, visualizer, paletteFromState ?? activityState, paletteBlend, animationFromState ?? activityState, animationBlend, paletteToState ?? paletteFromState ?? activityState, animationToState ?? animationFromState ?? activityState, fieldRows);
    return [
        centered(viewportMode === "wide" ? title : TITLE, w),
        row(`${sessionActive ? "↺" : idleActive ? "?" : errorActive ? "※" : toolActive ? "◇" : stateGlyph(attentionActive, thinking, transition)}  ${mood}  /  ${possessView ? possessTime : sessionActive ? sessionWeather : idleActive ? "idle" : errorActive ? weather : toolActive ? sensation : breath}  /  ${possessView ? possessDate : `${36 + (textFrame % 7)}°`}`, w),
        row(scan.repeat(Math.max(0, w - 14)), w),
        ...field.map((line) => row(line, w)),
        row(whisperLine, w),
        row(`${sessionActive ? sessionWeather : idleActive ? idleText : errorActive ? weather : toolActive ? sensation : memoryCount > 0 && textFrame % 9 < 3 ? memoryMark : echo}`, w),
        row(residue, w),
        row(`  ${trace}`, w),
    ];
}
export function headerPanel(...args) {
    return panel(...args);
}
export default function haunt(pi) {
    installCleanFooter(pi);
    let enabled = true;
    let visualizer = "braille";
    let possess = false;
    let tuiRef;
    let smooth = false;
    let textFrame = -1;
    ...
    let possessActivity = "waiting";
    let possessPalette = "waiting";
    ...
    const modeFor = (tui) => {
        ...
        if (!possess && tui.mode !== "fullscreen")
            return "haunt";
    ...
    const renderMachine = (width, rows = MACHINE_FIELD_ROWS) => {
        const state = viewState();
        ...
        const lines = headerPanel(width, visualFrame, previousFrame, state.attentionActive, attentionCount, thinking, echo, transition, toolActive, toolCount, state.errorActive, state.idleActive, idleCount, memoryCount, state.sessionActive, sessionCount, interaction, interactionStrength, knockSeed, whisper, visualizer, paletteActivity, paletteBlend, activity, animationActivity, animationBlend, paletteTarget ?? paletteActivity, animationTarget ?? animationActivity, rows, possess, textFrame);
        return lines.map((line) => `${colorFor(state)}${line}${PALETTE.reset}`);
    };
    const renderView = (tui, width) => {
        const mode = modeFor(tui);
        const fit = (line) => truncateToWidth(line, Math.max(0, width));
        ...
        if (possess) {
            ...
            return renderSpectre(width, rows, visualFrame, activity, attentionCount + toolCount + sessionCount, interaction, interactionStrength, whisper, paletteState ?? activity, paletteBlend, animationState ?? activity, animationBlend, paletteTarget ?? activity, animationTarget ?? activity, textFrame).map((line) => fit(`${colorFor(state)}${line}${PALETTE.reset}`));
        }
        if (mode === "machine")
            return renderMachine(width, Math.max(1, Math.min(MACHINE_HEIGHT, tui.terminal.rows - 12))).map(fit);
        ...
        const lines = compactPanel(width, visualFrame, state, interaction === "knock" ? knockSeed : attentionCount, interaction, interactionStrength, whisper, paletteState ?? state, paletteBlend, animationState ?? state, animationBlend, paletteTarget ?? state, animationTarget ?? state, textFrame);
        if (visualizer === "ascii") {
            const ascii = asciiCompactPanel(width, Math.floor(frame), compactState(), interaction === "knock" ? knockSeed : attentionCount, interaction);
        ...
        if (tui.terminal.rows <= 15)
            return lines.slice(0, 1).map((line) => fit(`${colorFor(viewState())}${line}${PALETTE.reset}`));
        return lines.map((line) => fit(`${colorFor(viewState())}${line}${PALETTE.reset}`));
    };
    const installResponsive = (ctx) => {
        if (headerInstalled || ctx.mode !== "tui")
            return;
        ...
        void ctx.ui.custom((tui, _theme, _keybindings, done) => {
            tuiRef = tui;
            refresh = () => tui.requestRender();
            overlayDone = () => done(undefined);
            return {
                render: (width) => renderView(tui, width),
                invalidate() {
                    tui.requestRender();
                },
            };
    ...
    const setRefreshTimer = () => {
        if (timer)
            clearInterval(timer);
        lastDrawAt = 0;
        timer = setInterval(draw, smoothActive() ? SMOOTH_RENDER_INTERVAL_MS : NORMAL_REFRESH_MS);
    };
    const draw = () => {
        if (!enabled || !current)
            return;
        ...
        if (interaction === "none" && Math.random() < (0.42 / 60) * tickScale) {
            interaction = ["knock", "wave", "approach", "carrier"][Math.floor(Math.random() * 4)];
        ...
        if (animationTarget) {
            if (smoothActive())
                animationBlend = Math.min(1, animationBlend + elapsed / SMOOTH_ANIMATION_INTERVAL_MS);
            else
                animationBlend = Math.min(1, animationBlend + 0.12);
            if (animationBlend === 1) {
        ...
        if (!paletteState)
            paletteState = nextPaletteState;
        if (nextPaletteState !== paletteState &&
        ...
        if (paletteTarget) {
            if (smoothActive())
                paletteBlend = Math.min(1, paletteBlend + elapsed / SMOOTH_ANIMATION_INTERVAL_MS);
            else
                paletteBlend = Math.min(1, paletteBlend + 0.12);
            if (paletteBlend === 1) {
        ...
        if (!smoothActive() || textFrame !== -1) {
            current.ui.setStatus("haunt", `${stateGlyph(state.attentionActive, thinking, transition)} ${activityLabel(activity)} / ${activityThought(activity, textFrame, attentionCount + toolCount)}`);
        }
        refresh?.();
    };
    ...
    pi.on("input", (event) => {
        if (!enabled || event.source === "extension" || !event.text.trim())
            return;
        lastActivity = Date.now();
        idleUntil = 0;
        memoryCount++;
    ...
    const handleCommand = async (args, ctx) => {
        ...
        if (command === "veil") {
            if (!possess)
                return ctx.ui.notify("the room is not open", "info");
        ...
        ctx.ui.notify("Usage: /haunt [toggle] | /knock|possess|veil|stir|smooth|whisper|braille|ASCII", "info");
    };
    ...
    for (const command of Object.keys(commandDescriptions)) {
        pi.registerCommand(command, {
            description: commandDescriptions[command] ?? commandDescriptions.animation,
            handler: async (args, ctx) => handleCommand(args.trim() ? `${command} ${args}` : command, ctx),
        });
    }
