...
function row(text, width) {
    ...
    return clip(`${" ".repeat(left)}${value}${" ".repeat(Math.max(0, width - left - visibleWidth(value)))}`, width);
}
function normalizeState(state) {
    if (state === "thinking")
        return "thinking";
    if (state === "writing" || state === "touching")
        return "writing";
    if (state === "reading" || state === "attention")
        return "reading";
    if (state === "error" || state === "fracturing")
        return "error";
    if (state === "session" || state === "re-entry")
        return "session";
    return "idle";
}
export function agentPhrase(state, frame) {
    const phrases = AGENT_PHRASES[normalizeState(state)];
    return phrases[Math.floor(frame / 3) % phrases.length];
}
function blendProfile(from, to, amount, paletteFrom = from, paletteAmount = amount, paletteTo = to) {
...
function pixelDensity(px, py, width, height, frame, profile, count, shape, interaction, interactionStrength, animationPool) {
    ...
    const zoom = interaction === "approach"
        ? 1 + easedStrength * (0.08 + (Math.sin(phase * 0.55) + 1) * 0.08)
        : 1;
    const warpedX = (x - Math.sin(y * 11 + phase * 1.3) * 0.045 * profile.noise) /
        inhale /
        zoom;
    const warpedY = y / breath / zoom;
    const carrierMix = interaction === "carrier" ? easedStrength : shape === "carrier" ? 1 : 0;
    const nearFace = shape === "near-face" ||
        (interaction === "approach" && strength > 0) ||
        interaction === "carrier";
    const envelope = Math.exp(-((warpedX + 0.08) ** 2 / 0.72 + (warpedY * 0.82) ** 2 / 1.2));
    const carrier = Math.exp(-((warpedY - Math.sin(warpedX * 8 + phase) * 0.18) ** 2) / 0.038);
    const counterWave = Math.exp(-((warpedY + 0.36 - Math.sin(warpedX * 12 - phase * 1.4) * 0.12) ** 2) /
        0.024);
    ...
    const memoryCarrier = Math.exp(-((memoryY - Math.sin(memoryX * 8 + memoryPhase) * 0.18) ** 2) / 0.05);
    const memory = memoryCarrier * (0.035 + Math.max(0, Math.sin(phase * 0.31 - 0.3)) * 0.045);
    const ringRadius = Math.sqrt(warpedX * warpedX + warpedY * warpedY);
    const ring = Math.exp(-((ringRadius - 0.55 - Math.sin(phase) * 0.025) ** 2) / 0.018);
    const pulse = Math.exp(-((ringRadius - (0.24 + ((phase % 6.28) / 6.28) * 0.58)) ** 2) / 0.012) * 0.22;
    // The exhale leaves a nearly subliminal pressure ring behind the signal.
    const pressure = Math.max(0, Math.sin(phase * 0.31 - 2.4)) ** 4;
    const breathRing = Math.exp(-((ringRadius - (0.72 + pressure * 0.18)) ** 2) / 0.045) *
        pressure *
        0.07;
    ...
    const eyeY = -0.18 + Math.sin(phase * 0.61) * 0.025 + Math.sin(phase * 0.23) * 0.012;
    const eyeGap = 0.3 + Math.sin(phase * 0.53) * 0.035;
    const eyeScale = 1 + Math.sin(phase * 0.57) * 0.1;
    const ellipse = (cx, cy, sx, sy) => Math.sqrt(((warpedX - cx) / sx) ** 2 + ((warpedY - cy) / sy) ** 2);
    const leftEye = ellipse(-eyeGap + faceTilt + gaze, eyeY, 0.23 * eyeScale, 0.105);
    const rightEye = ellipse(eyeGap + faceTilt + gaze, eyeY + 0.012, 0.23 * eyeScale, 0.105);
    const blink = Math.max(0, Math.sin(phase * 0.29 + 0.8)) ** 18;
    const eyeOpenness = 1 - blink * 0.92;
    const eyeRim = (Math.exp(-((leftEye - 1) ** 2) / 0.055) +
        Math.exp(-((rightEye - 1) ** 2) / 0.055)) *
        eyeOpenness;
    const eyeVoid = (Math.exp(-(leftEye ** 2) / 0.55) + Math.exp(-(rightEye ** 2) / 0.55)) *
        eyeOpenness;
    const leftBrow = Math.exp(-((warpedY - (-0.35 + ((warpedX + eyeGap) * 0.9) ** 2 * 0.18)) ** 2) /
        0.018) * Math.exp(-((warpedX + eyeGap) ** 2) / 0.22);
    const rightBrow = Math.exp(-((warpedY - (-0.35 + ((warpedX - eyeGap) * 0.9) ** 2 * 0.18)) ** 2) /
        0.018) * Math.exp(-((warpedX - eyeGap) ** 2) / 0.22);
    const bridge = Math.exp(-((warpedX -
        faceTilt * 0.5 -
        gaze * 0.45 -
        Math.sin(warpedY * 4 + phase) * 0.025) **
        2) / 0.012) * Math.exp(-((warpedY + 0.01) ** 2) / 0.36);
    const mouthY = 0.39 + Math.sin(phase * 0.49) * 0.035;
    const mouth = Math.exp(-((warpedY - mouthY - Math.sin(warpedX * 6 + phase) * 0.025) ** 2) /
        0.018) * Math.exp(-(warpedX ** 2) / 0.35);
    const jaw = Math.exp(-((Math.sqrt((warpedX / 0.72) ** 2 + ((warpedY - 0.03) / 0.9) ** 2) -
        0.82) **
        2) / 0.025);
    const cheek = Math.exp(-((warpedY - 0.1 - Math.sin(warpedX * 8 - phase) * 0.09) ** 2) / 0.03) * Math.max(0, 1 - Math.abs(warpedX) * 1.4);
    ...
    const figureX = (warpedX - 0.52 - Math.sin(phase * 0.17) * 0.035) / figureInhale;
    const figureY = warpedY / figureBreath;
    const figureTiltX = figureX - figureTurn * (figureY + 0.1);
    const figureHead = Math.exp(-((figureTiltX / 0.18) ** 2 + ((figureY + 0.34) / 0.24) ** 2)) *
        0.18;
    const figureShoulders = Math.exp(-((figureY - 0.02 - Math.sin(figureTiltX * 5 + phase) * 0.04) ** 2) /
        0.055) *
        Math.exp(-(figureTiltX ** 2) / 0.34) *
        0.16;
    const figureBody = Math.exp(-((figureTiltX / 0.34) ** 2 + ((figureY - 0.38) / 0.72) ** 2)) *
        0.07;
    const secondEyeLeft = Math.exp(-(((figureTiltX + 0.07) / 0.035) ** 2 + ((figureY + 0.34) / 0.025) ** 2)) * 0.11;
    const secondEyeRight = Math.exp(-(((figureTiltX - 0.07) / 0.035) ** 2 + ((figureY + 0.34) / 0.025) ** 2)) * 0.11;
    const peripheralFigure = emergence *
        (figureHead +
            figureShoulders +
            figureBody +
            secondEyeLeft +
    ...
    const stroke = (x1, y1, x2, y2, width) => {
        const dx = x2 - x1;
        const dy = y2 - y1;
        const along = Math.max(0, Math.min(1, ((warpedX - x1) * dx + (warpedY - y1) * dy) / (dx * dx + dy * dy)));
        return Math.exp(-(((warpedX - (x1 + along * dx)) ** 2 +
            (warpedY - (y1 + along * dy)) ** 2) /
            width));
    };
    const wave = interaction === "wave"
        ? (() => {
            const sway = Math.sin(phase * 0.9) * 0.09;
            const palmX = 0.5 + sway;
            const fingers = [-0.18, -0.09, 0, 0.09, 0.18].map((offset, index) => stroke(palmX + offset, 0.05, palmX + offset + sway * 0.35, [-0.62, -0.78, -0.84, -0.74, -0.58][index], 0.012));
            return Math.max(0, Math.min(1, Math.exp(-((warpedX - palmX) ** 2 / 0.08 + (warpedY - 0.12) ** 2 / 0.18)) *
                0.9 +
    ...
    const knock = interaction === "knock"
        ? (() => {
            ...
            if (animationPool === "small") {
                switch (kind) {
                    ...
                    case 5: // A flock of points crosses the field in a diagonal.
                        response =
                            [-0.55, -0.28, 0, 0.28, 0.55].reduce((sum, x) => sum + eye(x + cycle * 0.22 - 0.11, x * 0.7, 0.045, 0.045), 0) *
                                (0.25 + pulse * 0.7);
                        break;
                    ...
                    case 7: // A ladder of short bars climbs from the bottom.
                        response =
                            Array.from({ length: 5 }, (_, index) => stroke(-0.34 + index * 0.17, 0.9, -0.34 + index * 0.17, 0.9 - cycle * 1.5, 0.01)).reduce((sum, value) => sum + value, 0) *
                                (0.25 + pulse * 0.75);
                        break;
                    ...
                    case 9: // A small orbit circles once around the centre.
                        response =
                            Array.from({ length: 5 }, (_, index) => {
                                ...
                                return eye(Math.cos(angle) * 0.38, Math.sin(angle) * 0.38, 0.045, 0.045);
                            }).reduce((sum, value) => sum + value, 0) *
                                (0.2 + pulse * 0.8);
                        break;
            ...
            switch (kind) {
                ...
                case 3: // A cracked signal radiates out, then forgets its shape.
                    response =
                        [0, 1.57, 3.14, 4.71].reduce((sum, angle) => sum +
                            stroke(0, 0, Math.cos(angle + tremor) * (0.2 + cycle), Math.sin(angle + tremor) * (0.2 + cycle), 0.014), 0) *
                            (1 - cycle * 0.45);
                    break;
                case 4: // Something breathes directly against the glass.
                    response =
                        Math.exp(-((ringRadius - (0.35 + Math.sin(phase * 0.8) * 0.08)) ** 2) /
                            0.025) *
                            0.8 +
                ...
                case 5: // Several small eyes appear at once, never quite aligned.
                    response =
                        [-0.48, 0, 0.48].reduce((sum, x) => sum +
                            eye(x + tremor, Math.sin(phase + x) * 0.18, 0.09, 0.045), 0) *
                            (0.35 + pulse * 0.7);
                    break;
                case 6: // A handprint-like palm and fingers press from the near side.
                    response =
                        eye(-0.12 + tremor, 0.18, 0.26, 0.32) * 0.55 +
                            [-0.24, -0.12, 0, 0.12, 0.24].reduce((sum, x) => sum + stroke(x, 0.02, x + tremor, -0.62, 0.014), 0) *
                                (0.35 + pulse * 0.65);
                    break;
                case 7: // The response climbs upward like a body behind frosted glass.
                    response =
                        [-0.5, -0.25, 0, 0.25, 0.5].reduce((sum, x) => sum + stroke(x, 0.9, x + tremor, -0.9 + cycle * 0.7, 0.012), 0) *
                            (0.25 + pulse * 0.75);
                    break;
                ...
                case 9: // A spiral coils toward the operator instead of expanding away.
                    response =
                        Array.from({ length: 6 }, (_, index) => {
                            ...
                            return stroke(Math.cos(angle) * radius, Math.sin(angle) * radius, Math.cos(angle + 0.9) * (radius + 0.12), Math.sin(angle + 0.9) * (radius + 0.12), 0.014);
                        }).reduce((sum, value) => sum + value, 0) *
                            (0.25 + pulse * 0.75);
                    break;
                case 10: // The image is dragged sideways in strips, like a thing escaping.
                    response =
                        Array.from({ length: 7 }, (_, index) => Math.exp(-((warpedY - (index - 3) * 0.18) ** 2) / 0.012) *
                            (0.25 + Math.sin(phase + index) ** 2 * 0.7)).reduce((sum, value) => sum + value, 0) *
                            (0.25 + cycle * 0.7);
                    break;
        ...
        : 0;
    const faceDensity = envelope * (0.13 + carrier * 0.32 + counterWave * 0.18) +
        ...
        wave +
        knock;
    const carrierDensity = envelope * (carrier * 0.72 + counterWave * 0.54) +
        ring * 0.19 +
        pulse +
        memory +
        wave +
    ...
    const interference = Math.sin(warpedX * 29 + warpedY * 17 + phase * 2.2) * 0.16 * profile.noise;
    const scan = 0.7 + 0.3 * Math.sin(((py + frame * profile.speed * 0.55) * Math.PI) / 3);
    density = Math.max(0, density * scan + interference);
    const edgeLoss = Math.max(0, Math.abs(warpedX) - 0.58) * (0.08 + profile.noise * 0.05);
    const packetLoss = noise(px * 0.7, py * 0.9, frame / 3 + 31) < edgeLoss;
    if (packetLoss)
        density *= 0.3;
    const tear = Math.sin(warpedY * 33 + phase * 2.5) > 0.78 &&
    ...
    const palettePosition = Math.abs((warpedX + 1) * 1.8 +
        Math.sin(warpedY * 5 + phase) * 1.4 +
        phase * profile.noise);
    const paletteIndex = Math.floor(palettePosition) % profile.ink.length;
    const paletteBlend = smoothstep(palettePosition - Math.floor(palettePosition));
    const palette = profile.ink[paletteIndex] ?? profile.ink[0];
    const nextPalette = profile.ink[(paletteIndex + 1) % profile.ink.length] ?? palette;
    ...
    const brightness = 0.5 + Math.max(0, 1 - Math.abs(warpedX + 0.35)) * 0.42 + density * 0.42;
    return {
...
function brailleRow(width, rowIndex, fieldRows, frame, profile, count, shape, interaction, interactionStrength, animationPool) {
    ...
    for (let cellX = 0; cellX < width; cellX++) {
        ...
        for (const [dx, dy, bit] of BRAILLE_DOTS) {
            ...
            const current = pixelDensity(x, y, pixelWidth, pixelHeight, sourceFrame, profile, count, shape, interaction, interactionStrength, animationPool);
            ...
            const sample = frameBlend === 0
                ? current
                : (() => {
                    const next = pixelDensity(x, y, pixelWidth, pixelHeight, sourceFrame + 1, profile, count, shape, interaction, interactionStrength, animationPool);
                    return {
...
export function renderBrailleSubliminal(width, rows, frame, state, count, interaction = "none", interactionStrength = 1, whisper = false, animationPool = rows <= 5 ? "small" : "large", paletteFromState = state, paletteBlend = 1, animationFromState = state, animationBlend = 1, paletteToState = state, animationToState = state, textFrame = frame) {
    const field = renderBrailleField(width, rows, frame, state, count, "near-face", interaction, interactionStrength, animationPool, paletteFromState, paletteBlend, animationFromState, animationBlend, paletteToState, animationToState);
    if (whisper) {
        const profile = blendProfile(PROFILES[normalizeState(animationFromState)], PROFILES[normalizeState(animationToState)], animationBlend, PROFILES[normalizeState(paletteFromState)], paletteBlend, PROFILES[normalizeState(paletteToState)]);
        ...
        const word = glitchText(BRAILLE_WHISPERS[index], textFrame + index * 3, profile);
        const row = Math.floor(Math.abs(Math.sin((textFrame + count * 13) * 12.9898) * 43758.5453) %
            field.length);
        const start = Math.floor(Math.abs(Math.sin((textFrame + count * 7) * 78.233) * 43758.5453) %
...
export function renderBrailleField(width, rows, frame, state, count, shape = "carrier", interaction = "none", interactionStrength = 1, animationPool = rows <= 5 ? "small" : "large", paletteFromState = state, paletteBlend = 1, animationFromState = state, animationBlend = 1, paletteToState = state, animationToState = state, textFrame = frame) {
    const fieldWidth = Math.max(1, Math.floor(width || 1));
    const fieldRows = Math.max(1, Math.floor(rows || 1));
    const profile = blendProfile(PROFILES[normalizeState(animationFromState)], PROFILES[normalizeState(animationToState)], animationBlend, PROFILES[normalizeState(paletteFromState)], paletteBlend, PROFILES[normalizeState(paletteToState)]);
    return Array.from({ length: fieldRows }, (_, rowIndex) => brailleRow(fieldWidth, rowIndex, fieldRows, frame, profile, count, shape, interaction, interactionStrength, animationPool));
}
function glitchText(text, frame, profile) {
    return Array.from(text)
        .map((char, index) => char !== " " && (index * 7 + frame * profile.speed) % 17 < 1.2
        ? GLITCH[(index + frame) % GLITCH.length]
        : char)
        .join("");
}
export function renderSpectre(width, rows = 5, frame = 0, state = "idle", count = 0, interaction = "none", interactionStrength = 1, whisper = false, paletteFromState = state, paletteBlend = 1, animationFromState = state, animationBlend = 1, paletteToState = state, animationToState = state, textFrame = frame) {
    ...
    const field = renderBrailleSubliminal(w, rows, frame, phase, count, interaction, interactionStrength, whisper, rows <= 5 ? "small" : "large", paletteFromState, paletteBlend, animationFromState, animationBlend, paletteToState, animationToState);
    const signal = GLITCH.slice(0, Math.max(2, Math.min(12, 4 + (Math.floor(frame / 2) % 8)))).join("");
    const carrier = phase === "thinking"
        ? "folding"
        : phase === "writing"
            ? "emitting"
            : phase === "reading"
    ...
    if (w < 30)
        return [
            row(glitchText(`S P E C T R E // ${phrase.toUpperCase()}`, textFrame, profile), w),
            ...field,
            row(glitchText(` STATE:${carrier.toUpperCase()}  ENCOUNTERS:${count}  SIGNAL:${signal}`.toUpperCase(), textFrame, profile), w),
        ];
    return [
...
function scatterWhisper(line, word, start, width) {
    const chars = Array.from(word).slice(0, Math.max(1, width));
    const safeStart = Math.max(0, Math.min(start, Math.max(0, width - chars.length)));
    let output = "";
    let visible = 0;
    for (let index = 0; index < line.length;) {
        ...
        const replacement = visible >= safeStart && visible < safeStart + chars.length
...
export default function spectre(_pi) { }
