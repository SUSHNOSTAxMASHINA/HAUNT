...
const formatTokens = (value: number): string =>
	value < 1_000 ? `${value}` : value < 1_000_000 ? `${Math.round(value / 1_000)}k` : `${(value / 1_000_000).toFixed(1)}M`;

export function installCleanFooter(pi: ExtensionAPI): void {
	let requestRender = () => {};
	let balance: number | undefined;
	let balanceTimer: ReturnType<typeof setInterval> | undefined;
	let balanceAbort: AbortController | undefined;

	const refresh = () => requestRender();

	const updateBalance = async (ctx: ExtensionContext) => {
		try {
			...
			if (credits?.total_credits == null || credits.total_usage == null) return;
